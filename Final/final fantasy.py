...
class ModelTransform:
    def __init__(self):
        ...
        self.rotation_euler = [np.pi/2, 2*np.pi, 0.0]
        self.scale = 1.0
...
def compute_face_normal(v0, v1, v2):
    ...
    return normal / norm_length if norm_length > 0 else np.array([0, 0, 0])
...
def draw_triangle_gouraud_textured(matrix, z_buffer, transform,
    ...
    i0 = min(0, np.dot(vertex_normals[0], light_dir))
    i1 = min(0, np.dot(vertex_normals[1], light_dir))
    i2 = min(0, np.dot(vertex_normals[2], light_dir))
        ...
        for x in range(xmin, xmax + 1):
            ...
            if l0 >= 0 and l1 >= 0 and l2 >= 0:
                ...
                if z < z_buffer[y, x]:
                    ...
                    color = (
                        int(tex_color[0] * intensity * 0.8),
                        int(tex_color[1] * intensity * 0.8),
                        int(tex_color[2] * intensity * 0.8)
                    )

                    matrix[y, x] = color


def render_model(matrix, z_buffer, model_path, texture_path, transform):
    vertices, faces, texture_coords, texture_indices = load_model(model_path)
    texture = Image.open(texture_path)



    transformed_vertices = transform_vertices(vertices, transform)
    vertex_normals = compute_vertex_normals(transformed_vertices, faces)
    for i, face in enumerate(faces):
        ...
        vt0 = texture_coords[texture_indices[i][0] - 1] if texture_indices and texture_indices[i] else [0, 0]
        vt1 = texture_coords[texture_indices[i][1] - 1] if texture_indices and texture_indices[i] else [0, 0]
        vt2 = texture_coords[texture_indices[i][2] - 1] if texture_indices and texture_indices[i] else [0, 0]
...
if __name__ == "__main__":
    transform1 = ModelTransform()
    transform1.translation = [-0.5, -0.03, 500]

    transform2 = ModelTransform()
    transform2.translation = [25, -0.03, 500]
    transform2.scale=300.0
    render_model(matrix, z_buffer, "cat.obj", "Cat.jpg", transform1)
    render_model(matrix, z_buffer, "model_1.obj", "bunny-atlas.jpg", transform2)


    Image.fromarray(matrix).save("output1.png")
